...
Tensor AvgPool2d::operator()(const Tensor& x) {
    ...
    if(kernel_size_ == -1) y.reshape({B, C});
    return y;
}

void AvgPool2d::compute_local(int tgt_ycoord, int tgt_xcoord, float* tgt_ptr, float* src_ptr, int src_w, int src_h){
    ...
    *tgt_ptr /= kernel_size_ * kernel_size_;
#endif
}
